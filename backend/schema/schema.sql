...
-- *****************************-- Create schema for the app
CREATE SCHEMA IF NOT EXISTS storex;

-- ************************************** storex.facilities

CREATE TABLE storex.facilities
(
    facility_id bigserial NOT NULL,
    name        text NOT NULL,
    address     text,
...
(
    ...
    CONSTRAINT fk_units_facility FOREIGN KEY (facility_id)
        REFERENCES storex.facilities(facility_id)
);

CREATE INDEX idx_units_facility_id ON storex.units(facility_id);

-- ************************************** storex.customers

CREATE TABLE storex.customers
(
...
CREATE TABLE storex.agreements
(
    ...
    CONSTRAINT fk_agreements_customer FOREIGN KEY (customer_id)
        REFERENCES storex.customers(customer_id),
    CONSTRAINT fk_agreements_unit FOREIGN KEY (unit_id)
        REFERENCES storex.units(unit_id)
);

-- One active agreement per unit
CREATE UNIQUE INDEX uq_active_agreement_per_unit
    ON storex.agreements(unit_id)
    WHERE status = 'active';

CREATE INDEX idx_agreements_customer_id ON storex.agreements(customer_id);
CREATE INDEX idx_agreements_unit_id ON storex.agreements(unit_id);

-- ************************************** storex.invoices

CREATE TABLE storex.invoices
(
    ...
    CONSTRAINT fk_invoices_agreement FOREIGN KEY (agreement_id)
        REFERENCES storex.agreements(agreement_id)
);

CREATE INDEX idx_invoices_agreement_id ON storex.invoices(agreement_id);

-- ************************************** storex.payments

CREATE TABLE storex.payments
(
    ...
    CONSTRAINT fk_payments_invoice FOREIGN KEY (invoice_id)
        REFERENCES storex.invoices(invoice_id)
);

CREATE INDEX idx_payments_invoice_id ON storex.payments(invoice_id);

-- ************************************** storex.access_logs

CREATE TABLE storex.access_logs
(
...
CREATE INDEX idx_accesslogs_customer_id ON storex.access_logs(customer_id);
CREATE INDEX idx_accesslogs_unit_id ON storex.access_logs(unit_id);

-- ************************************** storex.messages

CREATE TABLE storex.messages
(
...
CREATE INDEX idx_messages_customer_id ON storex.messages(customer_id);
-- ************************************* app.units
